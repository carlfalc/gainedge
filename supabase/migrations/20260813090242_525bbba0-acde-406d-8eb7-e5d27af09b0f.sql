...
DECLARE
  ...
  v_id bigint;
BEGIN
  IF _fn NOT IN ('ron-snapshot', 'ron-label', 'ron-calibrate', 'ron-quality', 'ron-research', 'ron-robustness', 'ron-agent-session-structure') THEN
...
REVOKE EXECUTE ON FUNCTION public.ron_invoke_worker(text, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ron_invoke_worker(text, jsonb) TO service_role;
/**
 * Falconer v7 TP3 backtest panel — extracted verbatim from the former BacktestingPage
 * workspace switch so that nothing touching `falconer-backtest` or `falconer_backtest_runs`
 * is lost when `/dashboard/backtesting` becomes the Strategy Lab V2 surface.
 *
 * This is the ONLY UI for the legacy Falconer backtest endpoint and its stored run history.
 */
...
export default function FalconerBacktestPanel() {
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div>
    <div style={{ color: C.sec, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 5 }}>{label}</div>
    {children}
  </div>;
}

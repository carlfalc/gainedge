  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 9, width: "100%", flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 16.5, fontWeight: 700, color: "#34D399", lineHeight: 1.25 }}>
      ...
      {open && (
        <div style={{ marginTop: 12 }}>
            ...
            {([14, 30] as const).map(p => {
              ...
              return (
                <button
                  ...
                  style={{
                    padding: "4px 14px", borderRadius: 6, fontSize: 13, fontWeight: 700, cursor: "pointer",
          ...
          {loading ? (
            ...
            <div style={{ fontSize: 13, color: "#EF4444", padding: 10 }}>{error}</div>
          ) : !data || data.length === 0 ? (
            <div style={{ fontSize: 13, color: C.muted, padding: 10, fontStyle: "italic" }}>
...
function SummaryCard({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string; sub: string; color: string }) {
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, color, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>
        {icon}{label}
      </div>
      <div style={{ fontSize: 14, fontWeight: 800, color: C.text, fontFamily: "'JetBrains Mono', monospace", marginBottom: 2 }}>
        {value}
      </div>
  ...
  return (
    <div style={{ border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 10px" }}>
      ...
      >
        ...
        <span style={{ fontSize: 13, fontWeight: 800, color: C.text, fontFamily: "'JetBrains Mono', monospace", minWidth: 80, textAlign: "left" }}>
          {inst.symbol}
        </span>
        {hasAnyData && strongest ? (
          <span style={{ fontSize: 12, color: C.sec, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 12, color: C.muted, fontStyle: "italic" }}>No data in selected period</span>
        )}
          ...
          {SESSIONS.map(sess => {
            ...
            return (
              <div key={sess.key} style={{ paddingLeft: 8, borderLeft: `2px solid ${sess.color}` }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: sess.color, marginBottom: 2 }}>
                  {sess.label}
                </div>
                {sp.dataPoints === 0 ? (
                  <div style={{ fontSize: 12, color: C.muted, fontStyle: "italic" }}>No candle data</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    ...
                    {sp.bestBuyHourUtc !== null && sp.bestBuyPct > 50 && (
                      <div style={{ fontSize: 12, color: "#22C55E", fontFamily: "'JetBrains Mono', monospace" }}>
...
function LoadingRow({ period }: { period: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, color: C.muted, fontSize: 13 }}>
      <div style={{

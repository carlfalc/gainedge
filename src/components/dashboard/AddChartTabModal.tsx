        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {Object.entries(grouped).map(([cat, syms]) => (
            <div key={cat} className="mb-3">
              <div className="px-2 py-1 text-[12px] font-bold uppercase tracking-wider text-white/40">{cat}</div>
              <div className="grid grid-cols-3 gap-1.5 px-1">
                {syms.map((s) => (
                  <button
                    key={s}
                    onClick={() => setPicked(s)}
                    className={`px-2 py-2 rounded text-sm font-mono font-bold border transition-all ${
                      picked === s
          ...
          {filtered.length === 0 && (
            <div className="text-center text-sm text-white/40 py-8">No instruments match "{search}"</div>
          )}
        </div>

        <div className="flex items-center gap-2 px-4 py-3 border-t border-white/10">
          <div className="text-[13px] text-white/50 mr-auto">
            {picked ? <>Selected <span className="font-mono font-bold text-white">{picked}</span></> : "Pick an instrument…"}
          </div>
          <button
            disabled={!picked}
            onClick={() => handleAdd("manual")}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded text-sm font-semibold bg-blue-500/15 border border-blue-500/40 text-blue-400 hover:bg-blue-500/25 disabled:opacity-30 disabled:cursor-not-allowed transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded text-sm font-semibold bg-[#00CFA5]/15 border border-[#00CFA5]/40 text-[#00CFA5] hover:bg-[#00CFA5]/25 disabled:opacity-30 disabled:cursor-not-allowed transition"
          >

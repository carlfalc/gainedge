/**
 * Phase 2D.2g — four-agent orchestration integration tests with the OPTIONAL contextual
 * `cross_asset_correlation` specialist added to the accepted Session V2 + Calibration V1
 * + Pattern Context V1 trio.
 *
...
const TRACE = "ron-2d2g-four-agent-fixture";
...
const quartet = () => [session(), calibration(), pattern(), crossAsset()];

describe("2D.2g — fourth optional contextual specialist in Orchestrator V1", () => {
  it("EXPECTED_AGENTS_V1 is unchanged by this phase", () => {
    ...
    expect(agentSpec("cross_asset_correlation")!.authority_class).toBe("contextual");
    expect(agentSpec("cross_asset_correlation")!.source_health_authoritative).toBe(false);
    expect(evidenceTtlMinutes("cross_asset_correlation", "15m")).toBe(120);
  });

  it("the accepted trio alone remains CONTEXT_SUPPORTED", async () => {
    const { decision } = await synthesizeDecision([session(), calibration(), pattern()], CTX);
    expect(decision.state).toBe("CONTEXT_SUPPORTED");
  });

  it("adding a supported neutral cross-asset context retains CONTEXT_SUPPORTED", async () => {
    const { decision } = await synthesizeDecision(quartet(), CTX);
    expect(decision.state).toBe("CONTEXT_SUPPORTED");
    expect(decision.recommendation).toBe("context_only");
  });

  it("neutral cross-asset context does not erase the Session structural direction", async () => {
    const { decision } = await synthesizeDecision(quartet(), CTX);
    expect(decision.direction).toBe("short");
    expect(decision.disagreements.some((d) => d.kind === "directional")).toBe(false);
  });

  it("a blocked/critical cross_asset is NON-BINDING and cannot force DATA_BLOCKED", async () => {
    const { decision } = await synthesizeDecision([
      session(), calibration(), pattern(),
      crossAsset(15, {
  ...
  it("a missing cross_asset is not a missing expected specialist", async () => {
    const { decision } = await synthesizeDecision([session(), calibration(), pattern()], CTX);
    expect(decision.coverage.missing_expected_agents).toEqual([]);
    expect(decision.coverage.present_agents).not.toContain("cross_asset_correlation");
  });

  it("canonical evidence refs are ordered by authority rank with both contextual agents", async () => {
    const { decision } = await synthesizeDecision(quartet(), CTX);
  ...
  it("decision and explanation hashes are order-independent and replay-identical", async () => {
    const a = await synthesizeDecision(quartet(), CTX);
    const b = await synthesizeDecision([...quartet()].reverse(), CTX);
    const c = await reconstructDecision(quartet(), CTX);
  ...
  it("safety invariants hold with four agents", async () => {
    const { decision, explanation } = await synthesizeDecision(quartet(), CTX);

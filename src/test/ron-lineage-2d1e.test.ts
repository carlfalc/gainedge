...
import { RON_QUALITY_VERSION, RON_QUALITY_VERSION_V3 } from "../../supabase/functions/_shared/ron-data-quality";
import { RON_FEATURE_VERSION, RON_FEATURE_VERSION_V4 } from "../../supabase/functions/_shared/ron-features";

describe("Phase 2D.1e — recovered-source lineage versions", () => {
  it("bumps every downstream artifact version exactly once", () => {
    expect(RON_QUALITY_VERSION).toBe(4);
    expect(RON_FEATURE_VERSION).toBe(5);
  ...
  it("registers every contract under its own key and points current at v7", () => {
    for (const [k, c] of Object.entries(CALIBRATION_CONTRACTS)) {
      expect(c.calibration_version).toBe(Number(k));
    }
    expect(CALIBRATION_CONTRACT_CURRENT).toBe(CALIBRATION_CONTRACT_V7);
  });
